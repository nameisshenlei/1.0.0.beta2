...
public:
	...
	virtual HRESULT STDMETHODCALLTYPE RenderAudioSamples(bool preroll)
	{
		...
		delay = gst_audio_ring_buffer_delay(GST_AUDIO_RING_BUFFER_CAST(m_ringbuffer));
		...
		if (delay > max_delay) {
			...
			GST_LOG_OBJECT(m_ringbuffer->sink, "Delay %u > max delay %u", delay,
				max_delay);

			wait_time =
				gst_util_uint64_scale(delay - max_delay, GST_SECOND,
				GST_AUDIO_INFO_RATE(&spec->info));
			GST_LOG_OBJECT(m_ringbuffer->sink, "Waiting for %" GST_TIME_FORMAT,
...
gst_decklink_audio_sink_ringbuffer_delay(GstAudioRingBuffer * rb)
{
	...
	if (self->output) {
		if (self->output->output->GetBufferedAudioSampleFrameCount((unsigned int*)&ret) != S_OK)
			ret = 0;
	}

	GST_LOG_OBJECT(self->sink, "Delay: %u", ret);
...
GstAudioRingBufferSpec * spec)
{
	...
	GST_DEBUG_OBJECT(self->sink, "EnableAudioOutput %d", sample_depth);

	self->audioCallback = new GStreamerAudioOutputCallback(self);

	ret = self->output->output->SetAudioCallback((GStreamerAudioOutputCallback*)self->audioCallback);
	...
	spec->segtotal = gint(spec->buffer_time / spec->latency_time);
...
gst_decklink_audio_sink_ringbuffer_release(GstAudioRingBuffer * rb)
{
	...
	if (self->output) {
		...
		GST_DEBUG_OBJECT(self->sink, "Release ringbuffer 1");
		...
		GST_DEBUG_OBJECT(self->sink, "Release ringbuffer 2");

		self->output->output->SetAudioCallback(NULL);
		self->output->output->FlushBufferedAudioSamples();
		((GStreamerAudioOutputCallback*)(self->audioCallback))->Release();
		self->audioCallback = NULL;
		GST_DEBUG_OBJECT(self->sink, "FlushBufferedAudioSamples");
	}
	// free the buffer
	g_free(rb->memory);
	rb->memory = NULL;

	GST_DEBUG_OBJECT(self->sink, "Release ringbuffer end");
	return TRUE;
}
...
gst_decklink_audio_sink_ringbuffer_close_device(GstAudioRingBuffer * rb)
{
	...
	if (self->output) {
		...
		GST_DEBUG_OBJECT(self->sink, "DisableAudioOutput");
...
GstStateChange transition)
{
	...
	GstDecklinkAudioSinkRingBuffer *buf =
		GST_DECKLINK_AUDIO_SINK_RING_BUFFER_CAST(GST_AUDIO_BASE_SINK
		(self)->ringbuffer);
