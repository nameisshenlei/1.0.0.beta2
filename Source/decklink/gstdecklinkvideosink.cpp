      ...
      BMDOutputFrameCompletionResult result)
  {
    switch (result) {
      ...
      case bmdOutputFrameDisplayedLate:
		  GST_LOG_OBJECT(m_sink, "Late Frame %p", completedFrame);
        break;
      case bmdOutputFrameDropped:
		  GST_LOG_OBJECT(m_sink, "Dropped Frame %p", completedFrame);
        break;
      case bmdOutputFrameFlushed:
		  GST_LOG_OBJECT(m_sink, "Flushed Frame %p", completedFrame);
        break;
      default:
		  GST_LOG_OBJECT(m_sink, "Unknown Frame %p: %d", completedFrame,
            (gint) result);
        break;
    }
...
gst_decklink_video_sink_set_caps (GstBaseSink * bsink, GstCaps * caps)
{
  ...
  self->videoCallback = new GStreamerVideoOutputCallback(self);
  self->output->output->SetScheduledFrameCompletionCallback((GStreamerVideoOutputCallback*)self->videoCallback);
...
gst_decklink_video_sink_prepare (GstBaseSink * bsink, GstBuffer * buffer)
{
  ...
  GST_LOG_OBJECT (self, "Preparing buffer %p", buffer);
  ...
  ret = self->output->output->CreateVideoFrame (self->info.width,
	  self->info.height, self->info.stride[0], bmdFormat8BitBGRA,
  ...
  for (i = 0; i < self->info.height; i++) {
    memcpy (outdata, indata, GST_VIDEO_FRAME_WIDTH (&vframe) * 4);
...
gst_decklink_video_sink_close (GstBaseSink * bsink)
{
  ...
  if (self->output) {
	...
	((GStreamerVideoOutputCallback*)(self->videoCallback))->Release();
    ...
    GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_PAUSED_TO_READY:
      ...
      self->output->clock_offset = 0;
      g_mutex_unlock (&self->output->lock);
	  GST_DEBUG_OBJECT(self, "paused to ready");
      break;
    case GST_STATE_CHANGE_PLAYING_TO_PAUSED:{
	  ...
	  GST_DEBUG_OBJECT(self,"Stopped scheduled playback");
	  break;
    }
    case GST_STATE_CHANGE_PAUSED_TO_PLAYING:{
      g_mutex_lock (&self->output->lock);
    ...
    GstQuery * query)
{
  ...
  size = (guint)(GST_VIDEO_INFO_SIZE (&info));
