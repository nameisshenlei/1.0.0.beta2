...
static const GstDecklinkMode modes[] = {
  ...
  {bmdModeHD1080p24, 1920, 1080, 30, 1, false, HD},
  ...
  {bmdModeHD1080i50, 1920, 1080, 30, 1, true, HD},
  ...
  {bmdModeHD1080p6000, 1920, 1080, 30, 1, false, HD},

  {bmdModeHD720p50, 1280, 720, 30, 1, false, HD},
...
gst_decklink_mode_get_structure (GstDecklinkModeEnum e)
{
  ...
  /*"interlace-mode", G_TYPE_STRING,
	  mode->interlaced ? "interleaved" : "progressive" , "colorimetry", G_TYPE_STRING,
...
public:
      ...
      IDeckLinkDisplayMode * mode, BMDDetectedVideoInputFormatFlags)
  {
    ...
    m_input->input->EnableVideoInput (mode->GetDisplayMode (),
		bmdFormat8BitBGRA, bmdVideoInputEnableFormatDetection);
...
static void gst_decklink_open_device(Device& device, bool is_input)
{
	HRESULT ret;
	if (is_input)
	{
		ret = device.input.device->QueryInterface(IID_IDeckLinkInput,
			(void **)&device.input.input);
		if (ret != S_OK) {
			GST_WARNING("selected device does not have input interface");
		}
		else {
			device.input.clock = gst_decklink_clock_new("GstDecklinkInputClock");
			GST_DECKLINK_CLOCK_CAST(device.input.clock)->input = &device.input;
			device.input.input->SetCallback(new GStreamerDecklinkInputCallback(&device.input));
		}
		ret = device.input.device->QueryInterface(IID_IDeckLinkConfiguration,
			(void **)&device.input.config);
		if (ret != S_OK) {
			GST_WARNING("selected device does not have config interface");
		}

		ret = device.input.device->QueryInterface(IID_IDeckLinkAttributes,
			(void **)&device.input.attributes);
		if (ret != S_OK) {
	...
	{
		ret = device.output.device->QueryInterface(IID_IDeckLinkOutput,
			(void **)&device.output.output);
		if (ret != S_OK) {
			...
			GST_DECKLINK_CLOCK_CAST(device.output.clock)->output = &device.output;
		}
	}
}
static void gst_decklink_close_device(Device& device, bool is_input)
{
	if (is_input)
	{
		device.input.attributes->Release();
		device.input.config->Release();
		g_object_unref(device.input.clock);
		device.input.input->Release();

		device.input.attributes = NULL;
		device.input.config = NULL;
		device.input.input = NULL;
		device.input.clock = NULL;
	}
	else
	{
		g_object_unref(device.output.clock);
		device.output.output->Release();

		device.output.clock = NULL;
...
init_devices (gpointer data)
{
  ...
  g_thread_new ("decklink_device_thread",(GThreadFunc) gst_decklink_com_thread, NULL);
...
gst_decklink_acquire_nth_output (gint n, GstElement * sink, gboolean is_audio)
{
  ...
  if (!output->output)
  {
	  if (output->device)
	  {
		  gst_decklink_open_device(devices[n], false);
	  }
	  else
	  {
...
gst_decklink_release_nth_output (gint n, GstElement * sink, gboolean is_audio)
{
  ...
  if (output->videosink == NULL && output->audiosink == NULL)
  {
	  gst_decklink_close_device(devices[n], false);
  }
  g_mutex_unlock (&output->lock);
}
...
gst_decklink_acquire_nth_input (gint n, GstElement * src, gboolean is_audio)
{
  ...
  if (!input->input) 
  {
	  if (input->device)
	  {
		  gst_decklink_open_device(devices[n],true);
	  }
	  else
	  {
...
gst_decklink_release_nth_input (gint n, GstElement * src, gboolean is_audio)
{
  ...
  if (input->audiosrc == NULL && input->videosrc == NULL)
  {
	  gst_decklink_close_device(devices[n], true);
  }
  g_mutex_unlock (&input->lock);
}
...
gboolean gst_decklink_plugin_init (GstPlugin * plugin)
{
