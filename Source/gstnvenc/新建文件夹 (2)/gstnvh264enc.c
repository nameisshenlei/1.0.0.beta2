...
_get_supported_profiles (GstNvH264Enc * nvenc)
{
  ...
  nv_ret =
	  GstNvEncGetEncodeProfileGUIDCount(GST_NV_BASE_ENC(nvenc)->encoder,
      NV_ENC_CODEC_H264_GUID, &n);
  if (nv_ret != NV_ENC_SUCCESS)
    return FALSE;

  nv_ret =
	  GstNvEncGetEncodeProfileGUIDs(GST_NV_BASE_ENC(nvenc)->encoder,
...
gst_nv_h264_enc_open (GstVideoEncoder * enc)
{
  ...
  /* Check if H.264 is supported */
  {
	...
	GstNvEncGetEncodeGUIDs(GST_NV_BASE_ENC(nvenc)->encoder, guids,
...
_get_interlace_modes (GstNvH264Enc * nvenc)
{
  ...
  if (GstNvEncGetEncodeCaps (GST_NV_BASE_ENC (nvenc)->encoder,
...
#define N_BYTES_SPS 128
  ...
  nv_ret = GstNvEncGetSequenceParams (GST_NV_BASE_ENC (nvenc)->encoder, &spp);
  if (nv_ret != NV_ENC_SUCCESS) {
    GST_ELEMENT_ERROR (nvenc, STREAM, ENCODE, ("Encode header failed."),
        ("GstNvEncGetSequenceParams return code=%d", nv_ret));
    return FALSE;
  }

  if (seq_size < 8) {
    GST_ELEMENT_ERROR (nvenc, STREAM, ENCODE, ("Encode header failed."),
        ("GstNvEncGetSequenceParams returned incomplete data"));
    return FALSE;
  }
    ...
    GstVideoCodecState * old_state, GstVideoCodecState * state)
{
  ...
  nv_ret =
	  GstNvEncGetEncodePresetConfig(GST_NV_BASE_ENC(h264enc)->encoder,
  ...
  if (old_state) {
    nv_ret =
        GstNvEncReconfigureEncoder (GST_NV_BASE_ENC (h264enc)->encoder,
        &reconfigure_params);
  } else {
    nv_ret =
        GstNvEncInitializeEncoder (GST_NV_BASE_ENC (h264enc)->encoder, params);
  }
